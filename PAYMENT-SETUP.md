...
3. Add `STREAMELEMENTS_ACCOUNT_ID` and `STREAMELEMENTS_JWT` through Supabase Edge Function Secrets. Use the credentials for the connected StreamElements channel that owns the public tipping page. In the current ⁅𝐓𝐡𝐲𝐓☣︎𝐱𝐢𝐜𝐆𝐚𝐦𝐞𝐫⁆ setup, that is the YouTube-linked channel; Twitch remains the viewer sign-in and request identity. Never place the private JWT in GitHub.
