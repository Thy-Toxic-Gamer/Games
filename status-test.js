(function(){
  ...
  get("cancel-request-form").addEventListener("submit",(event)=>{event.preventDefault();const r=read();if(!r||r.status!=="pending"){cancelDialog.close();render();return}r.status="cancelled";r.cancelledAt=new Date().toISOString();save(r);cancelDialog.close();render()});
